...
The runner resolves and applies the effective phase before entering its foreground loop. Manual and fixed modes recheck every 15 seconds and sun mode every 60 seconds; the checks sleep between invocations and do not busy-loop. A night instance restores the saved day state when procd stops it, including during disable or reload. A day instance leaves the LEDs restored while remaining observable by procd. The CLI retains failed snapshots for explicit recovery. Same-phase Night reconciles arrivals and checks ownership; a failed apply publishes an unknown phase and a recovery marker. The marker remains visible after a failed stop, while `/var/run/led-nightmode/phase` is removed on stop. See the [restoration contract](restoration-contract.md) for the supported schema, identity limits and recovery procedure.
