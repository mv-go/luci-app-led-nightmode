...
The repository includes the hardware-validated CLI core, a UCI/procd service, fixed-time and sunrise/sunset scheduling, an rpcd/ACL interface, and an extensible provider interface for indicators outside the Linux LED class. Release `0.3.0-r1` builds as two `noarch` APKs with the official OpenWrt 25.12.4 `mediatek/filogic` SDK: the universal base package and an optional Quectel QNWCFG provider. The previous `0.1.0-r3` release was live-validated on the first BPI-R3 Mini: a `day → night → day → night` cycle switched off all nine sysfs LEDs and its RM520N-GL LTE indicator, restored both classes during the day, and kept LTE/5G and SSH available.
...
The `luci.led-nightmode` rpcd object exposes status and phase resolution to read-authorized LuCI sessions. Manual phase changes, service reloads, and explicit provider probes require write authorization. The RPC layer does not expose arbitrary commands or accept provider paths outside the fixed driver directory.

The LuCI view remains a later milestone.
